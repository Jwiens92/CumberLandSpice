...
/********************************************************/
	...
	function layerslider_enqueue_content_res() {
		...
		$pluginPath = get_stylesheet_directory_uri() . '/plugins/LayerSlider';
	 
		wp_enqueue_script('layerslider_js', $pluginPath . '/js/layerslider.kreaturamedia.jquery.js', array('jquery'), '3.5.0' );
		wp_enqueue_script('jquery_easing', $pluginPath . '/js/jquery-easing-1.3.js', array('jquery'), '1.3.0' );
		wp_enqueue_style('layerslider_css', $pluginPath . '/css/layerslider.css', array(), '3.5.0' );
	}
...
/********************************************************/
	...
	function layerslider_enqueue_admin_res() {
		...
		// Global
		wp_enqueue_style('layerslider_global_css', $pluginPath . '/css/global.css', array(), '3.5.0' );
		...
		if(strstr($_SERVER['REQUEST_URI'], 'layerslider')) {
			...
			wp_enqueue_script('layerslider_admin_js', $pluginPath . '/js/admin.js', array('jquery'), '3.5.0' );
			wp_enqueue_style('layerslider_admin_css', $pluginPath . '/css/admin.css', array(), '3.5.0' );
...
function layerslider_settings_menu() {
    ...
    $layerslider_hook = add_menu_page('LayerSlider WP', 'LayerSlider WP', 'manage_options', 'layerslider', 'layerslider_router', get_stylesheet_directory_uri() . '/plugins/LayerSlider/img/icon_16x16.png');
...
function layerslider_admin_notice() {
    ...
    <div id="layerslider_notice" class="updated">
        <img src="<?php echo get_stylesheet_directory_uri() . '/plugins/LayerSlider/img/ls_80x80.png' ?>" alt="WeatherSlider icon">
	...
	foreach($sample_slider as $sliderkey => $slider) {
		...
		foreach($sample_slider[$sliderkey]['layers'] as $layerkey => $layer) {
			...
			if(!empty($sample_slider[$sliderkey]['layers'][$layerkey]['properties']['background'])) {
				$sample_slider[$sliderkey]['layers'][$layerkey]['properties']['background'] = $pluginPath . '/sampleslider/' . $layer['properties']['background'];
			}

			// Change thumbnail images if any
			if(!empty($sample_slider[$sliderkey]['layers'][$layerkey]['properties']['thumbnail'])) {
				$sample_slider[$sliderkey]['layers'][$layerkey]['properties']['thumbnail'] = $pluginPath . '/sampleslider/' . $layer['properties']['thumbnail'];
			}
			...
			foreach($layer['sublayers'] as $sublayerkey => $sublayer) {
				...
				if($sublayer['type'] == 'img') {
					$sample_slider[$sliderkey]['layers'][$layerkey]['sublayers'][$sublayerkey]['image'] = $pluginPath . '/sampleslider/' . $sublayer['image'];
				}
			}
		}
	}
